const getAvailedSubscription = async (id) => {
  try {
    const response = await fetch(`/api/v1/subscriptionsAvailed/${id}`);
...
const updateSubscriptionsBooking = async (data, id) => {
  try {
    const response = await fetch(`/api/v1/bookings-subscription/${id}`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    alert("An error occurred while updating the subscription booking.");
  }
};
...
async function showPopupSubscription(subscriptionId) {
  ...
  subscriptionBookingIdToUpdate = availedSubscription.data.subscriptionUserAvailed.bookingId;
  plateNumberToUpdate = availedSubscription.data.subscriptionUserAvailed.plateNumber;
  ...
  const servicesInsideSubscription = availedSubscription.data.subscriptionUserAvailed.subscriptionDetails;
...
function generateSubscriptionPopup(services) {
  ...
  const subscriptionPlateNumber = document.getElementById("subscriptionPopupPlateNumber");
  ...
  for (service of services) {
    ...
    subscriptionPopupServiceName.textContent = `${service.service
      .toLowerCase()
      .replace(/\b\w/g, (char) => char.toUpperCase())} Token Remaining Tokens: ${service.tokensAmount}`;
    ...
    strongElement.textContent = `Price: 1 ${service.service.toLowerCase().replace(/\b\w/g, (char) => char.toUpperCase())} Token`;
    ...
    subscriptionPlateNumber.insertAdjacentElement("afterend", subscriptionPopupServiceName);

    counter++;
  }
  const lastPopupContent = document.querySelector('.popupContent[data-service-id="0"]');
  ...
  const radioButtons = document.querySelectorAll('input[type="radio"][name="subscriptionSelected"]');
...
function showDateTimePopupSubscription() {
  document.getElementById("subscriptionPopup").style.display = "none";
  document.getElementById("dateTimePopupSubscription").style.display = "block";
}
...
async function submitDateTimeSubscriptionBooking() {
  ...
  await updateSubscriptionsBooking(newBookingSubscription, subscriptionBookingIdToUpdate);
