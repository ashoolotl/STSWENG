...
const generateTokenForUser = async (newBookingId) => {
  ...
  const serviceExists = services.some((service) => service.name === booking.product);
...
exports.webhookIsStillSubscribed = catchAsync(async (req, res, next) => {
  ...
  let event;
  try {
    event = stripe.webhooks.constructEvent(req.body, sig, process.env.STRIPE_WEBHOOK_INVOICE_PAID_SECRET);
  } catch (err) {
    res.status(400).send(`Webhook Error: ${err.message}`);
    return;
  }
