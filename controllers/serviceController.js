...
exports.resizeServicePhoto = (req, res, next) => {
  ...
  sharp(req.file.buffer).resize(3200, 1800).toFormat("jpeg").jpeg({ quality: 90 }).toFile(`public/images/services/DEFAULT.jpeg`);

  next();
};

exports.validateServiceData = catchAsync(async (req, res, next) => {
  console.log("INSIDE VALIDATE SERVICE DATA");
  ...
  if (name) {
    ...
    const serviceNameAlreadyExist = services.some((service) => {
      return service.name.toUpperCase() === serviceNameUpperCase && service._id.toString() !== req.params.serviceId;
    });
    if (serviceNameAlreadyExist) {
      return next(new AppError(`The ${serviceNameUpperCase} service is already registered. Please choose another service name`, 400));
    }
  }
  if (prices) {
    // check if the input is a valid vehicle classification
    const vehicleClassificationList = await VehicleClassification.find().distinct("name");
...
exports.updateSubscriptionWithService = catchAsync(async (req, res, next) => {
  ...
  await Subscription.updateMany(
    ...
    {
      arrayFilters: [{ "price.services.service": service.name }, { "elem.service": service.name }],
